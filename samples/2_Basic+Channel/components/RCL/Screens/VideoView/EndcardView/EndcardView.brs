...
' README:
' EndcardView is a RCL component to render Endcards 
' No business logic is made in Endcard view
' User should set all fields outside and handle fields triggered
Sub Init()
    ' focusedChild used internally to handle first time shown - what item on screen should be shown
    ...
    ' there could be use cases when view is created but not shown, so user should
    ' start countdown explicitly
    m.top.ObserveField("startTimer", "OnStartTimerSet")

    ' m.FocusableGroup used to check is user focused on screen
    ...
    ' isEndcardFirstFocus Used to know is this a first shown of the screen to handle focus
    ...
    if m.LastThemeAttributes <> invalid then
        ...
        RCL_SetBackgroundTheme(m.LastThemeAttributes)
    end if
End Sub
