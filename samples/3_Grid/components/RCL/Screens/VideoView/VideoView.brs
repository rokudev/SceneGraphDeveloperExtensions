...
' README:
' VideoView is a RCL component to play video items or playlists
' In channel user create VideoView,
...
sub Init()
    ...
    ' internal flag to know did we already load content for endcard screen or not
...
sub OnContentSet(event as Object)
    ' User should implement own OnContentSet to update the buttons etc.
end sub
...
sub OnVideoStateChanged(event as Object)
    ...
    if m.video <> Invalid then
        ...
        if not handled and statesToClose[state] <> Invalid then
            ...
            if errorCode <> 0
                ? "[RCL] video.errorCode == ";m.video.errorCode; " video.errorMsg == "; m.video.errorMsg
            end if

            ' This field is observed by library, so it will close this screen and show previous
            ' if you set this field to non top screen it will just remove it from stack
            m.top.close = true
        end if
    end if
end sub
...
sub OnEndcardTimerFired()  ' Don't insert event as parameter to use this callback as function.
    ...
    if m.time = 0
        ...
        if hasNextItemInPlaylist
            ...
            m.video.SetFocus(true)
        else ' if there is no video available, close the screen
            m.top.close = true
        end if
    end if
end sub
...
sub ShowEndcardView()
	' We should delete HandlerConfigEndcard only if endcard screen was shown
...
sub RCL_SetTheme(theme as Object)
    ...
    RCL_setThemeFieldstoNode(m, themeAttributes, theme)
end sub
