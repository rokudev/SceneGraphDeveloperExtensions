...
sub InitContentGetterValues()
    ...
    ' to prefetch data when user will need it
    m.EXTRA_ROWS = 2
    m.VISIBLE_ITEMS = 5
...
'@param content - [AA] - content that will be passed to user
...
'This is utility function to create components defined in user channel
...
'reset all idle values
'is called in init and when user navigate so we start new background loading based on focused position
sub resetIdleValues()
