...
sub StartLoadingContent()
    if m.view <> invalid and m.view.content <> invalid and m.view.content[m.Handler_ConfigField] <> invalid and m.view.content[m.Handler_ConfigField].name <> invalid and m.view.content[m.Handler_ConfigField].name.Len() > 0 then
        ' we shouldn' t check children count as user might set new config to refresh screen 
...
function getHorizontalpaginationCallback(row, HandlerConfigGrid, itemIndexToCheck, page)
    ...
    return {
        ...
        ' there is no way to restore page number if user called replace
        onError: sub(data)
            ...
            ' when user moves focus number of fails is cleared for all rows
        ...
        updatePageNumber: sub(restoreDefault as Boolean)
            ...
            ' check 2x items in case user modifies more items that he stated in config
        ...
        if not IsToManyPageFails(row, -1)
            ...
            callback = {
                ...
                onreceive: function(data)
                    'we shouldn' t do anything here as user should populate all content here
                    m.row.isLoaded = true
                ...
                onError: function(data)
                    ...
                    ' restore previous CG config if user didn' t set new one so we can retry later
...
'This is called after m.top.IDLE_ROW_LOAD_TIME when user doesn' t navigate on screen 
