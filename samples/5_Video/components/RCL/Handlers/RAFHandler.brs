...
sub PlayContentWithFullRAFIntegration()
    ...
    adIface.rcl_original_importAds = adIface.importAds
    adIface.importAds = sub(ads)
            m.rcl_flag_importAds_was_called = true
            m.rcl_importedAds = ads
            m.rcl_original_importAds(ads)
        end sub

    adIface.rcl_original_StitchedAdsInit = adIface.StitchedAdsInit
    adIface.StitchedAdsInit = sub(ads)
            m.rcl_flag_StitchedAdsInit_was_called = true
            m.rcl_original_StitchedAdsInit(ads)
        end sub

    ' User can configure Raf via overrided ConfigureRAF function inside Handler in channel
    ConfigureRAF(adIface)

    ' Should user scene as a place for ads rendering
    view = m.top.getScene()
    if view = invalid
    ...
    if adIface.rcl_flag_importAds_was_called = Invalid and adIface.rcl_flag_StitchedAdsInit_was_called = Invalid then
        adPods = adIface.GetAds()

    ' if it is imported ads, get it to show in preroll
    else if adIface.rcl_flag_importAds_was_called = true
        adPods = adIface.rcl_importedAds
    end if
    ...
    playContent = true
    if adPods <> invalid and adPods.count() > 0 and adIface.rcl_flag_StitchedAdsInit_was_called = Invalid
    ...
    else ' if preroll was skipped, close video screen
    ...
    while true
        ...
        curAd = Invalid
        if adIface.rcl_flag_StitchedAdsInit_was_called <> Invalid and adIface.rcl_flag_StitchedAdsInit_was_called = true
        ...
        ' if it is some video playback event
            ...
            if adIface.rcl_flag_StitchedAdsInit_was_called = Invalid
            ...
            end if ' adIface.rcl_flag_StitchedAdsInit_was_called = Invalid
        end if
    end while
