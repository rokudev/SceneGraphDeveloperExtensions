...
sub Init()
    ...
    m.itemsList.focusBitmapUri = "pkg:/components/RCL/focus.9.png"
...
' number of items that have to be populated in order to show this screen
    ...
    if content <> invalid
        if content.GetChildCount() > 0
            if content.GetChild(0).GetChildCount() > 0
                ...
                if m.debug then ? "RCL: Category List view received content with empty category"
                return false
            end if
        else
            if m.debug then ? "RCL: Category List view received empty content"
            return false
        end if
    else
        if m.debug then ? "RCL: Category List view received invalid content"
        return false
    end if
end function
