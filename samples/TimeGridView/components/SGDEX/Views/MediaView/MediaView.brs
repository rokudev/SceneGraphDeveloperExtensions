...
sub OnFocusedChildChange()
    if m.media <> invalid
        ...
        if m.top.wasShown and not m.endcardAvailable
            if GetCurrentMode() = "video" and m.isButtonBarVisible
                if m.renderOverContent    ' renderOverContent = true
                    ...
                    if m.trickplayVisible
                        ' show buttonBar if playback paused || trickplay visible
                        m.buttonBar.opacity = 1.0
                    else
        ...
        if m.buttonBar.IsInFocusChain() and m.isButtonBarVisible and m.renderOverContent and m.top.IsInFocusChain() = false
...
sub ProcessEndState()
    ...
    if m.RafTask = invalid and GetState() = "finished"
        HandlerConfigEndcard = currentItem.HandlerConfigEndcard
...
sub OnPausedPlayback()
    m.trickplayVisible = true
    if GetCurrentMode() = "audio" and m.isButtonBarVisible then
        m.buttonBar.opacity = 1.0
    end if
end sub
